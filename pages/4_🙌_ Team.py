import streamlit as st
import pandas as pd

st.set_page_config(page_title="Team", page_icon="🙌", layout="wide")

st.markdown("UI and Deployement Team")
st.write("BSc AI Level 5, University of Gloucestershire")

st.markdown("## Data")
st.write("- Robbie Smith, Building UI interface and intergrating models to UI for deployment")

st.markdown("## Data")
st.write("- Brandon Elward, Building knowledge base for models to train from the data colected")

st.markdown("## Product")
st.write("- Lilia Vladova, Team Lead of UI and Development")

st.markdown("## Documentation")
st.write("- Name 3, One Line Contribution")
